...
int main(int argc, char **argv) {
	...
	if (argc < 11) {
		...
		fprintf(stderr, "\tencrypted seed backup (32 bytes or empty to generate a new seed)\n");
		fprintf(stderr, "\tuser entropy (32 bytes or empty, must be empty if restoring a seed)\n");
		fprintf(stderr, "\tseed encryption key (16 bytes or empty)\n");
		return 0;
	}
	operationModeFlag = 0;
	arg = strtok(argv[1], "+");
	...
	if (strlen(argv[10]) == 0) {
		...
		if (seedLength != 0) {
			fprintf(stderr, "Seed encryption key is mandatory when a seed backup is provided\n");
			return 0;			
		}
	}
	else {		
		...
		if (developerKeyLength == 0) {
			fprintf(stderr, "Invalid seed encryption key\n");
			return 0;
		}
		if (developerKeyLength != sizeof(developerKey)) {
			fprintf(stderr, "Invalid seed encryption key length\n");
			return 0;
		}
	}	
	initDongle();
