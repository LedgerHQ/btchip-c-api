...
const char * QWERTY_KEYMAP = "00271E1F202122232425260405060708090A0B0C0D0E0F101112131415161718191A1B1C1D372C28";
const char * AZERTY_KEYMAP = "03271E1F202122232425261405060708090A0B0C0D0E0F331112130415161718191D1B1C1A362C28";
...
int main(int argc, char **argv) {
	...
	int wipePinLength;
	unsigned char keymapEncoding[40];
	...
	if (argc < 11) {
		...
		fprintf(stderr, "\tFeatures flag combined with + (RFC6979)\n");
	...
	if ((hexFeaturesFlag & 0x02) != 0) {
		operationModeFlag |= 0x80;
	}
	if (strlen(argv[3]) == 0) {
		argv[3] = "00";
	}
	...
	displayBinary(out + apduSize, 32);
	apduSize += 32;	
	printf("Server HOTP operation key : ");
	displayBinary(out + apduSize, 48);
	apduSize += 48;
